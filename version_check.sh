...
chart_app_version=$(yq eval .appVersion kubernetes/cray-dhcp-kea/Chart.yaml)
chart_image_version=$(cat kubernetes/cray-dhcp-kea/Chart.yaml |grep image:|cut -d ':' -f3)
chart_values_global_chart_version=$(yq eval .global.chart.version kubernetes/cray-dhcp-kea/values.yaml)
chart_values_global_app_version=$(yq eval .global.appVersion kubernetes/cray-dhcp-kea/values.yaml)
